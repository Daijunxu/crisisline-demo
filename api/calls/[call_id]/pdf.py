...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        try:
            ...
            if len(path_parts) >= 4 and path_parts[-1] == 'pdf':
                call_id = path_parts[-2]
            else:
                call_id = "CALL-2025-002"  # Default
            ...
            if not call_data:
                ...
                self.end_headers()
                self.wfile.write(json.dumps({"error": "Call not found"}).encode())
                return
            
            # Generate PDF
            pdf_content = generate_call_report(call_data)
